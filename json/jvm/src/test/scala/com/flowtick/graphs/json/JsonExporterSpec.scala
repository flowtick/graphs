...
class JsonExporterSpec extends AnyFlatSpec with Matchers {
  ...
  val numberGraph: Graph[EdgeType, Int] = Graph.fromEdges(Set(1 --> 2)).withNode(Node.of(3))

  "JSON export" should "export graph with schema" in {
    implicit val nodeJson = new JsonWithSchema[Int, SchemaType] {
      override def json(value: Int): Json = Json.obj("value" -> Json.fromInt(value))
      override def schema(value: Int): Schema[Unit] =
        if (value == 1) Schema($id = Some("root")) else Schema($id = Some("number"))
    }
    ...
    val exported = JsonExporter.exportJson[EdgeType, Int, SchemaType](numberGraph, Schema($id = Some("root")))

    exported should be(Valid(
      Json.obj(
