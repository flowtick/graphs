...
final case class Styles[S](classes: Option[Map[String, S]] = None,
                           overrides: Option[Map[String, S]] = None)(implicit monoid: StyleMonoid[S]) {
  def getStyle(id: Option[String],
               classList: List[String] = List.empty)(default: Option[S]): S =
    (default ++
      classList.flatMap(classes.getOrElse(Map.empty).get) ++
      id.flatMap(overrides.getOrElse(Map.empty).get)).foldLeft(monoid.empty)(monoid.combine)
  ...
  def ++(styles: Styles[S]): Styles[S] =
    copy(overrides = Some((styles.overrides ++ overrides).reduce(_ ++ _)), classes = Some((styles.classes ++ classes).reduce(_ ++ _)))
}
...
trait StyleSheetLike {
  ...
  def getNodeStyle(id: Option[String], classList: List[String] = List.empty): Option[NodeShape]
  def getEdgeStyle(id: Option[String], classList: List[String] = List.empty): Option[EdgeShape]

  def requireNodeStyle(id: Option[String], classList: List[String] = List.empty): NodeShape =
    getNodeStyle(id, classList).getOrElse(NodeShape())

  def requireEdgeStyle(id: Option[String], classList: List[String] = List.empty): EdgeShape =
    getEdgeStyle(id, classList).getOrElse(EdgeShape())

  def images: Map[String, ImageSpec]

  def updateNodeStyle(id: String, nodeShape: Option[NodeShape] => Option[NodeShape]): StyleSheetLike
  def updateEdgeStyle(id: String, edgeShape: Option[EdgeShape] => Option[EdgeShape]): StyleSheetLike
...
final case class StyleSheet(id: Option[String] = None,
                            nodes: Option[Styles[NodeShape]] = None,
                            edges: Option[Styles[EdgeShape]] = None,
                            images: Map[String, ImageSpec] = Map.empty) extends StyleSheetLike {
  override lazy val nodeStyles: Styles[NodeShape] = nodes.getOrElse(Styles[NodeShape]()(StyleMonoid.nodeStyles))
  override lazy val edgeStyles: Styles[EdgeShape] = edges.getOrElse(Styles[EdgeShape]()(StyleMonoid.edgeStyles))
  ...
  def getEdgeStyle(id: Option[String], classList: List[String] = List.empty): Option[EdgeShape] = {
    edges.map(_.getStyle(id, classList)(None))
  }

  def getNodeStyle(id: Option[String], classList: List[String] = List.empty): Option[NodeShape] = {
    nodes.map(_.getStyle(id, classList)(None))
  }
...
final case class StyleSheets(styleSheets: List[StyleSheet]) extends StyleSheetLike {
  override lazy val nodeStyles: Styles[NodeShape] = styleSheets.view.map(_.nodeStyles).reduce(_ ++ _)
  override lazy val edgeStyles: Styles[EdgeShape] = styleSheets.view.map(_.edgeStyles).reduce(_ ++ _)

  override def id: Option[String] = None

  override def getNodeStyle(id: Option[String], classList: List[String]): Option[NodeShape] =
    styleSheets.map(_.getNodeStyle(id, classList)).find(_.isDefined).flatten

  override def getEdgeStyle(id: Option[String], classList: List[String]): Option[EdgeShape] =
  ...
  private def updateSheet(filter: StyleSheet => Boolean)(update: StyleSheet => StyleSheet): StyleSheets = {
  ...
  override def updateNodeStyle(id: String, nodeShape: Option[NodeShape] => Option[NodeShape]): StyleSheetLike =
    updateSheet(_.nodeStyles.overrides.exists(_.contains(id)))(_.updateNodeStyle(id, nodeShape))

  override def updateEdgeStyle(id: String, edgeShape: Option[EdgeShape] => Option[EdgeShape]): StyleSheetLike =
    updateSheet(_.edgeStyles.overrides.exists(_.contains(id)))(_.updateEdgeStyle(id, edgeShape))

  override def withImage(id: String, imageSpec: ImageSpec): StyleSheetLike =
    updateSheet(_.images.contains(id))(_.withImage(id, imageSpec))

  override def merge(other: List[StyleSheet]): StyleSheetLike =
    copy(styleSheets ++ other.filterNot(_.id.exists(styleSheets.flatMap(_.id).contains)))
}
