...
final case class Completed[S](state: S, backtrack: Option[S] = None) extends TraversalEvent[S] {
  override def toString: String =
    s"Completed(state = $state, backtracking: ${backtrack.isDefined} ($backtrack))"
}
...
object Traversal {
  ...
  final case class Step[E, N](
      node: Node[N],
      edge: Option[Edge[E]] = None,
  ...
  def nodes[E, N](graph: Graph[E, N])(initialNodes: Iterable[Node[N]])(
    ...
    val initialState = initialNodes.foldLeft(state)((currentState, node) =>
      currentState.add(Step(node, depth = Some(0)))
    )
...
trait TraversalState[S, Id] {
  ...
  /** trigger the completion after the state returned
    * @param f
    *   transformation of the current state, should ne used to add next states
    * @param completionState
    * @return
    */
  def triggerCompletion(f: TraversalState[S, Id] => TraversalState[S, Id])(
      completionState: S
  ): TraversalState[S, Id]
}
...
private final case class StackBasedState[S, Id](
    stack: List[S],
    markers: Map[Id, Traversal.Marker[S]] = Map.empty[Id, Traversal.Marker[S]]
) extends TraversalState[S, Id] {
  override def mark(
      state: Id,
      marker: Traversal.Marker[S]
  ): TraversalState[S, Id] =
  ...
  override def next: (Option[S], TraversalState[S, Id]) =
    (
      stack.headOption,
      copy(stack = if (stack.isEmpty) List.empty else stack.tail)
    )
  override def add(nodeState: S): TraversalState[S, Id] =
    copy(stack = nodeState +: stack)

  override def triggerCompletion(
      f: TraversalState[S, Id] => TraversalState[S, Id]
  )(completionState: S): TraversalState[S, Id] =
    f(add(completionState))
}

private final case class QueueBasedState[S, Id](
    queue: Queue[S],
    markers: Map[Id, Traversal.Marker[S]] = Map.empty[Id, Traversal.Marker[S]]
) extends TraversalState[S, Id] {
  override def mark(
  ...
  override def next: (Option[S], TraversalState[S, Id]) =
    queue.dequeueOption
      .map { case (node, remaining) =>
        (Some(node), copy(queue = remaining))
      }
      .getOrElse((None, this))
