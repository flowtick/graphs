...
final case class GraphMLEdgeShape(
    edgeStyle: Option[EdgeShape] = None,
    labelValue: Option[String] = None,
    path: Option[EdgePath] = None
)

final case class EdgeValues(
    edgeEdge: Option[GraphMLEdgeShape] = None,
...
class GraphMLEdgeDatatype[T](edgeDatatype: Datatype[T]) extends Datatype[GraphMLEdge[T]] {
  ...
  override def keys(targetHint: Option[String]): Seq[GraphMLKey] =
    edgeDatatype.keys(targetHint) ++ Seq(
      GraphMLKey(
        id = "edge_graphics",
        targetHint = targetHint,
        yfilesType = Some("edgegraphics")
      )
    )
  ...
  override def deserialize(
      ...
      targetHint: Option[String]
  ): ValidatedNel[Throwable, GraphMLEdge[T]] =
    (for {
      ...
      val edgeValues = properties.foldLeft(EdgeValues()) {
        case (edge, property)
            if graphKeys
              .get(property.key)
              .exists(_.yfilesType.exists(_ == "edgegraphics")) =>
          edge.copy(edgeEdge = extractEdgeShape(property))
        case (edge, property)
            if isValueProperty(
              property,
              graphKeys,
              edgeDatatype.keys(Some("edge")).map(_.id)
            ) =>
      ...
      GraphMLDatatype.singleAttributeValue("id", edgeNode) match {
        case Some(id) =>
          edgeDatatype
            .deserialize(edgeValues.valueXml.reverse, graphKeys, targetHint)
            .map { value =>
              GraphMLEdge[T](
                id,
                value,
        ...
        case None =>
          invalidNel(
            new IllegalArgumentException(
              s"unable to parse id from edge ${from.toString}"
            )
          )
      }
    }).getOrElse(
      invalidNel(
        new IllegalArgumentException(
          s"unable to parse GraphML edge from ${from.toString}"
        )
      )
    )
  ...
  protected def extractEdgeShape(
      property: GraphMLProperty
  ): Option[GraphMLEdgeShape] = property.xml.nonEmptyChildren.headOption
    ...
    .map { dataNode =>
      dataNode.child.foldLeft(GraphMLEdgeShape(Some(EdgeShape()))) {
        case (shape, elem) if elem.label == "EdgeLabel" =>
          ...
          shape.copy(
            edgeStyle = shape.edgeStyle.map(_.copy(labelStyle = Some(label))),
            labelValue = Some(elem.text.trim)
          )
