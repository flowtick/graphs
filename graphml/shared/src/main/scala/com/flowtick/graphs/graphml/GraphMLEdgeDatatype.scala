...
class GraphMLEdgeDatatype[V, Repr <: HList, FromRepr <: HList](implicit
  ...
  override def serialize(edge: GraphMLEdge[V]): NodeSeq = {
    ...
    val edgeValueData = genericValueKeys().runtimeList.zip(genericValue.to(edge.value).runtimeList).map {
      case (key: Symbol, value) if classOf[Integer].isAssignableFrom(value.getClass) =>
        <data key={ s"edge_${key.name}" } type="integer">{ value.toString }</data>

      case (key: Symbol, value) if classOf[Double].isAssignableFrom(value.getClass) =>
        <data key={ s"edge_${key.name}" } type="double">{ value.toString }</data>

      case (key: Symbol, value) =>
        <data key={ s"edge_${key.name}" } type="string">{ value.toString }</data>
  ...
  override def deserialize(from: NodeSeq, graphKeys: collection.Map[String, GraphMLKey]): ValidatedNel[Throwable, GraphMLEdge[V]] =
    ...
    }).getOrElse(invalidNel(new IllegalArgumentException(s"unable to parse GraphML edge from $from")))
