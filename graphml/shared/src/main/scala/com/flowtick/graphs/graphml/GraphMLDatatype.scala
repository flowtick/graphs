...
private[graphml] object PartialParsedGraph {
  def empty[N]: PartialParsedGraph[N] = PartialParsedGraph[N](None, scala.collection.mutable.Map.empty, List.empty)
...
final case class GraphMLProperty(key: String, xml: XmlNode, typeHint: Option[String] = None) {
  def nodeValue: Object = if (xml.child.exists(!_.isInstanceOf[Text])) xml.child else xml.child.text
}
...
class GraphMLDatatype[E, N](nodeDataType: Datatype[GraphMLNode[N]],
  ...
  override def serialize(g: GraphMLGraph[E, N], targetHint: Option[String]): NodeSeq = {

    def graphKeys: Iterable[XmlNode] = (nodeDataType.keys(nodeTargetHint) ++ edgeDataType.keys(edgeTargetHint) ++ g.meta.keys).map { key: GraphMLKey =>
      // format: OFF
      <key id={ key.id }
    ...
    def edgesXml: Iterator[XmlNode] = g.graph.edges.iterator.flatMap(edge => edgeDataType.serialize(edge.value, edgeTargetHint))
    def nodesXml: Iterator[XmlNode] = g.graph.nodes.iterator.flatMap(node => nodeDataType.serialize(node.value, nodeTargetHint))
  ...
  override def deserialize(
    ...
    targetHint: Option[String]): ValidatedNel[Throwable, GraphMLGraph[E, N]] =
    from.headOption match {
      case Some(root) =>
        val resources: Seq[GraphMLResource] = GraphMLDatatype
          .parseProperties(root)
          .filter(property => graphKeys.get(property.key).exists(key => key.yfilesType.contains("resources") && key.targetHint.contains("graphml")))
          .flatMap(property => property.xml.nonEmptyChildren.headOption
  ...
  protected def parseGraphRoot(
    graph: XmlNode,
    graphKeys: scala.collection.Map[String, GraphMLKey],
    resources: Seq[GraphMLResource]): Validated[NonEmptyList[Throwable], GraphMLGraph[E, N]] =
    parseGraphNodes(graph, graphKeys).andThen { parsedGraph =>
      parseEdges(parsedGraph.edgesXml, parsedGraph.nodes, graphKeys).andThen { edges =>
        valid(GraphMLGraph(Graph(edges, parsedGraph.nodes.values), GraphMLMeta(parsedGraph.id, graphKeys.values.toSeq, resources)))
      }
    }
  ...
  protected def mergePartialGraphs(graphs: Iterator[ValidatedNel[Throwable, PartialParsedGraph[N]]]): ValidatedNel[Throwable, PartialParsedGraph[N]] =
  ...
  protected def parseNode(nodeXml: scala.xml.Node, graphKeys: scala.collection.Map[String, GraphMLKey]): Validated[NonEmptyList[Throwable], PartialParsedGraph[N]] = {
    nodeDataType.deserialize(Seq(nodeXml), graphKeys, nodeTargetHint) match {
      case Valid(node) => mergePartialGraphs(nodeXml.child.iterator.map {
        ...
        case _ => valid(PartialParsedGraph(None, Map(node.id -> Node.of(node)), List.empty))
      })
  ...
  protected def parseGraphNodes(graphNode: scala.xml.Node, graphKeys: scala.collection.Map[String, GraphMLKey]): ValidatedNel[Throwable, PartialParsedGraph[N]] = {
    ...
    val partialGraphs: Iterator[Validated[NonEmptyList[Throwable], PartialParsedGraph[N]]] = graphNode.child.iterator.zipWithIndex.map {
...
object GraphMLDatatype {
  ...
  protected[graphml] def parseProperties(node: XmlNode): Iterator[GraphMLProperty] =
  ...
  protected[graphml] def parseResources(resourcesNode: XmlNode): Iterator[GraphMLResource] =
    resourcesNode.child.iterator.flatMap {
      case data if data.label == "Resource" =>
        for {
          id <- GraphMLDatatype.singleAttributeValue("id", data).filter(_.nonEmpty)
        } yield GraphMLResource(
          id,
          data.text,
          GraphMLDatatype.singleAttributeValue("type", data).filter(_.nonEmpty).orElse(Some("svg"))
        )
      case _ => None
    }

  protected[graphml] def parseKeys(rootElem: scala.xml.Node): scala.collection.Map[String, GraphMLKey] =
    rootElem.child.iterator.filter(_.label.toLowerCase == "key").flatMap { keyElem =>
