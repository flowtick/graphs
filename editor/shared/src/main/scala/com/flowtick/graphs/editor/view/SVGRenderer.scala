...
/** a renderer for svg like models
  ...
  * @tparam M
  */
abstract class SVGRenderer[Builder, T <: FragT, FragT, M](
    protected val bundle: Bundle[Builder, T, FragT],
    protected val options: SVGRendererOptions
)(implicit
    appender: T => generic.Frag[Builder, FragT],
    matrixLike: SVGMatrixLike[M]
) {
  import bundle.all._
  ...
  def renderGraph[E, N](
      ...
      edgeStyleRef: StyleRef[Edge[E]]
  ): IO[SVGRenderer[Builder, T, FragT, M]] = {
    ...
    for {
      ...
      _ <- graph.edges
        .map(renderEdge(_, graph, layout, styleSheet))
        .toList
        .sequence
    } yield this
  }
  ...
  def renderNode[E, N](
      ...
      nodeLabel: Labeled[N, String],
      nodeStyleRef: StyleRef[GraphNode[N]]
  ): IO[SVGNodeElement[T]] = for {
    nodeElement <- IO {
      val geometry = layout.nodeGeometry(node.id)
      val shape = styleSheet.requireNodeStyle(
        nodeStyleRef.id(node),
        nodeStyleRef.classList(node)
      )
      ...
      val shapeElement: T =
        shape.svgContent.flatMap(content => styleSheet.images.get(content.refId)) match {
          case Some(image) if image.imageType == ImageType.svg =>
            parseSvg(image.data)

          case _ =>
            shapeTag(shape, styleSheet, width, height)(
  ...
  def renderLabel(
    ...
    val textCenterY = (height / (1 + lines.size)) - Try(
      labelStyle.fontSize.getOrElse(defaultFontSize).toDouble * 0.6
    ).getOrElse(10.0)
  ...
  def renderEdge[E, N](
      ...
      edgeLabel: Labeled[E, String],
      edgeStyleRef: StyleRef[Edge[E]]
  ): IO[Option[SVGEdgeElement[T]]] = for {
    edgeElementOpt <- IO {
      for {
        ...
        style = styleSheet
          .getEdgeStyle(edgeStyleRef.id(edge), edgeStyleRef.classList(edge))
          .getOrElse(EdgeShape())
        ...
        label = {
          ...
          val bendPoints =
            layout.edgePath(edge.id).map(_.points).filter(_.nonEmpty)
  ...
  protected def renderPanZoomRect: Option[T] = Some(
    svg
      .rect(
        id := "pan-zoom-hit",
        svgAttrs.width := "100%",
  ...
  def shapeTag(
      ...
      widthValue: Double,
      heightValue: Double
  ): generic.TypedTag[Builder, T, FragT] = {
    import bundle.all._
    import bundle.{svgAttrs, svgTags => svg}
