...
class ModelUpdateFeature extends EditorComponent {
  ...
  implicit val jsonDataType: Datatype[Json] =
    EditorModel.jsonDataType(emptyOnError = true)

  val handleAddNode: Transform = ctx =>
    ctx.transform { case addNode: AddNode =>
      val nodeToAdd = addNode.stencilRef
  ...
  val handleAddEdge: Transform = ctx =>
    ctx.transform { case edge: AddEdge =>
      addEdge(edge)(ctx)
    }
  ...
  val load: Transform = ctx =>
    ctx.transform {
      case Load(xml, GraphMLFormat) =>
        FromGraphML[Json, Json](xml) match {
          case Right(graphml) =>
            val schemaFromGraphOrDefault: Option[EditorModel.EditorSchema] = {
              import io.circe.generic.auto._

              for {
                schemaResource <- graphml.meta.resources.find(
                  _.typeHint.contains("schema")
                )
                schema <- JsonSchema
                  .parse[EditorSchemaHints](schemaResource.value)
                  .toOption
              } yield schema
            }

            val withNodes = graphml.graph.nodes.foldLeft(ctx.model) { case (model, node) =>
              val withNodeAndLayout = model
                ...
                .updateLayout(currentLayout => {
                  node.value.geometry
                    .map(nodeGeometry => currentLayout.setNodeGeometry(node.id, nodeGeometry))
                    .getOrElse(currentLayout)
                })

              node.value.shape match {
                case Some(shape) =>
                  withNodeAndLayout.updateStyleSheet(
                    _.updateNodeStyle(node.id, _ => Some(shape))
                  )
                case None => withNodeAndLayout
              }
            }

            val withNodesAndEdges = graphml.graph.edges.foldLeft(withNodes) { case (model, edge) =>
              edge.value.shape
                .map(edgeShape =>
                  model
                    .updateStyleSheet(
                      _.updateEdgeStyle(edge.id, _ => Some(edgeShape))
                    )
                    .updateLayout(current =>
                      edge.value.path
                        .map(edgePath => current.setEdgePath(edge.id, edgePath))
                        .getOrElse(current)
                    )
                    .updateGraph(
                      _.withEdge(
            ...
            val withGraphAndStyle =
              graphml.resourcesById.foldLeft(withNodesAndEdges) {
                case (model, (key, GraphMLResource(_, value, typeHint))) =>
                  typeHint match {
                    case Some("java.awt.image.BufferedImage") =>
                      model.updateStyleSheet(
                        _.withImage(
                          key,
                          ImageSpec(
                            s"data:image/png;base64,${unescapeXml(value)
                              .replaceAll("\n", "")}",
                            "dataUrl"
                          )
                        )
                    ...
                    case _ =>
                      model.updateStyleSheet(
                        _.withImage(key, ImageSpec(unescapeXml(value), "svg"))
                      )
                  }
              }
              ...
              .updateModel(model =>
                model.copy(
                  ...
                  styleSheet = StyleSheets(
                    editorGraph.styleSheets.flatMap(_.toOption)
                  ) merge model.config.styleSheets,
                  schema = EditorSchemas(
                    editorGraph.schemas.flatMap(_.toOption)
                  ) merge model.config.schemas
                )
              )
              .addNotification(this, Reset)
  ...
  val handleReset: Transform = ctx =>
    ctx.transform { case Reset =>
      ...
      case Export(GraphMLFormat) =>
        ...
        ctx.addError(
          ...
          new UnsupportedOperationException(
            "graphml export not supported right now"
          )
        )
      case Export(JsonFormat) =>
        val json = EditorGraphJsonFormat.defaultEditorGraphEncoder(
          EditorGraph(
            graph = ctx.model.graph,
            styleSheets = ctx.model.styleSheet.styleSheets.map(Right(_)),
            layouts = ctx.model.layout.toGraphLayouts.map(Right(_)),
            schemas = ctx.model.schema.schemas.map(Right(_))
          )
        )
  ...
  def graphWithSchema(
      graph: GraphMLGraph[Json, Json],
      schema: EditorModel.EditorSchema
  ): GraphMLGraph[Json, Json] = {
    import io.circe.generic.auto._
    ...
    val schemaResource = GraphMLResource(
      ...
      Printer.noSpaces
        ...
        .print(JsonSchema.toJson[EditorSchemaHints](schema)),
      Some("schema")
    )
    graph.addResource(schemaResource)
  }

  val handleSelect: Transform = ctx =>
    ctx.transform {
      case SelectAll =>
        val allNodes =
  ...
  private def withSelection(
      ctx: EditorContext,
      selection: Set[ElementRef],
      append: Boolean
  ): EditorContext = {
  ...
  val handleConnectToggle: Transform = ctx =>
    ctx.transform { case EditorToggle(EditorToggle.connectKey, Some(toggle)) =>
      ctx.copy(model = ctx.model.copy(connectSelection = toggle))
    }
    ...
    ctx.transform { case DeleteSelection =>
      ctx.model.selection.foldLeft(ctx) {
        case (elementCtx, ElementRef(refId, NodeType)) =>
          ...
          val withoutEdges = edges.foldLeft(elementCtx) { case (edgeContext, edge) =>
            edgeContext
