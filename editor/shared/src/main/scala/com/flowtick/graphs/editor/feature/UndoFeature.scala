  ...
  override def eval: Eval = ctx =>
    ctx
      .effect(this) {
        ...
        case ElementUpdated(ElementRef(_, _), updateType, _) if updateType != Internal =>
          IO {
            if (
              modelHistory.isEmpty || modelHistory.headOption
                .exists(lastModel => lastModel.version < ctx.model.version)
            ) {
              modelHistory.prepend(ctx.model)
            }
          }
      }
      .flatMap(_.transformIO { case Undo =>
        IO {
          if (modelHistory.length > 1) {
            ...
            modelHistory.headOption match {
              case Some(previousModel) =>
                ctx.addCommand(
                  SetModel(previousModel.copy(selection = Set.empty))
                )
              case None => ctx
            }
