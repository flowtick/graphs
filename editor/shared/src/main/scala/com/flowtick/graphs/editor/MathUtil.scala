...
object MathUtil {
  ...
  def segmentIntersect(a: LineSegment,
                       b: LineSegment,
                       considerCollinearOverlapAsIntersect: Boolean = false): Option[Vector2] = {
    val p = a.start
    val p2 = a.end
    val q = b.start
    val q2 = b.end
    ...
    if (isZero(rxs) && isZero(qpxr))
    {
      ...
      if (considerCollinearOverlapAsIntersect && ((0 <= (q - p)*r && (q - p)*r <= r*r) || (0 <= (p - q)*s && (p - q)*s <= s*s))) {
  ...
  def rectIntersect(segment: LineSegment, rect: Rectangle): Option[Vector2] = {
    // both outside?
    val bothOut = !pointInRect(segment.start, rect) && !pointInRect(segment.end, rect)
    val bothIn = pointInRect(segment.start, rect) && pointInRect(segment.end, rect)

    if (bothIn || bothOut) {
      None
    } else segmentIntersect(segment, rect.top)
