...
final case class EditorConfiguration(palettes: Option[List[Palette]] = None) {
  lazy val styleSheets: List[StyleSheet] =
    palettes.getOrElse(List.empty).map(_.styleSheet)
  lazy val schemas: List[Schema[EditorSchemaHints]] =
    palettes.getOrElse(List.empty).map(_.schema)
}

object EditorConfiguration {
  implicit val schemaHintsCodec: Codec[EditorSchemaHints] =
    deriveCodec[EditorSchemaHints]

  implicit val configDecoder: Decoder[EditorConfiguration] =
    new Decoder[EditorConfiguration] {
      override def apply(c: HCursor): Result[EditorConfiguration] = for {
        palettes <- {
          import io.circe.generic.auto._
          implicit val schemaDecoder: Decoder[Schema[EditorSchemaHints]] =
            com.flowtick.graphs.json.schema.JsonSchema.jsonSchemaDecoder
