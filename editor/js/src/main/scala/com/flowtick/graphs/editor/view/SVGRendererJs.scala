...
class SVGRendererJs(options: SVGRendererOptions)
  ...
  override def x(elem: dom.Element): Double =
    elem.asInstanceOf[SVGLocatable].getCTM().e
  ...
  override def unselectElement(graphElement: GraphElement[Element]): IO[Unit] =
    IO {
  ...
  override def deleteElement(element: GraphElement[Element]): IO[Unit] = for {
    ...
    _ <- IO(
      element.selectElem.foreach(selectElem => selectElem.parentNode.removeChild(selectElem))
    ).attempt.void
...
object SVGRendererJs {
  implicit val domSVGMatrix: SVGMatrixLike[SVGMatrix] =
    new SVGMatrixLike[SVGMatrix] {
      private val matrixSvg =
        JsDom.svgTags.svg.render // unattached svg for independent calculations
      ...
      override def translate(
          matrix: SVGMatrix
      )(dx: Double, dy: Double): SVGMatrix = matrix.translate(dx, dy)
      override def scale(matrix: SVGMatrix)(factor: Double): SVGMatrix =
        matrix.scale(factor)
  ...
  def apply(options: SVGRendererOptions = SVGRendererOptions()) =
    new SVGRendererJs(options)
}
