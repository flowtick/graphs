...
class EditorPropertiesJs(containerId: String)(val messageBus: EditorMessageBus) extends EditorProperties {
  ...
  override def toggleEdit(enabled: Boolean): IO[Boolean] = IO(EditorPropertiesHtml.propertiesToggle.click()).attempt.map(_ => true)

  override def setPropertiesGroups(properties: List[PropertySpec], elementProperties: ElementProperties): IO[List[PropertyFormGroup]] = for {
