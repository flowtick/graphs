...
class EditorMenuJs(menuContainerId: String)(val messageBus: EditorMessageBus) extends EditorMenu {
  ...
  lazy val menuContainer =
    org.scalajs.dom.window.document.getElementById(menuContainerId)
  ...
  lazy val fileInput: Input =
    input(`type` := "file", id := "file", onchange := readFile).render
  ...
  def iconTag(icon: String): JsDom.TypedTag[html.Element] = i(
    cls := s"fa fa-$icon"
  )

  val navbar: Div = div(
    cls := "collapse navbar-collapse",
    id := "menuNavbarCollapse",
    ul(
  ...
  override def handleExported(exported: ExportedGraph): IO[Unit] =
    showLink(exported.name, exported.value, exported.format)
