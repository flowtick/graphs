...
class SVGRendererJvm(options: SVGRendererOptions)(implicit
    ...
    matrixLike: SVGMatrixLike[Affine],
    xmlDom: XmlDom
  ...
  override protected def applyTransformation(transformation: Affine): Unit = {
    ...
    graphSVG.viewPort.setAttribute(
      "transform",
      s"matrix(${transformation.getMxx} 0 0 ${transformation.getMyy} ${transformation.getTx} ${transformation.getTy})"
    )
    Try(
      setDimensions(
...
object SVGRendererJvm {
  ...
  private implicit val matrixLike: SVGMatrixLike[Affine] =
    new SVGMatrixLike[Affine] {
      ...
      override def tx_=(matrix: Affine)(value: Double): Unit =
        matrix.setTx(value)

      override def ty(matrix: Affine): Double = matrix.getTy
      override def ty_=(matrix: Affine)(value: Double): Unit =
        matrix.setTy(value)

      override def scalex(matrix: Affine): Double = matrix.getMxx
      override def scalex_=(matrix: Affine)(value: Double): Unit =
        matrix.setMxx(value)

      override def scaley(matrix: Affine): Double = matrix.getMyy
      override def scaley_=(matrix: Affine)(value: Double): Unit =
        matrix.setMyy(value)
