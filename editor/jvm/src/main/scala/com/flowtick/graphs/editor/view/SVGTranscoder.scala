...
object SVGTranscoder {
  def svgXmlToPng(svgXml: String,
                  scaleWidth: Option[Double],
                  scaleHeight: Option[Double]): IO[Array[Byte]] =
    transcodeSvgToPng(new ByteArrayInputStream(svgXml.getBytes("UTF-8")), scaleWidth, scaleHeight)

  def transcodeSvgToPng(input: InputStream,
                        scaleWidth: Option[Double],
                        scaleHeight: Option[Double]): IO[Array[Byte]] =
    IO(input).bracket(data => IO {
      val dpi = 100
