...
class ModelUpdateFeatureSpec extends EditorBaseSpec {
  ...
  "Editor Model Update" should "add nodes and edges" in withEditor { editor =>
    ...
    loaded.model.layout.nodeGeometry(firstNodeId) should be(
      Some(DefaultGeometry(100.0, 100.0, 50.0, 50.0))
    )
    loaded.model.layout.edgePath(edgeId) should be(
      Some(EdgePath(25.0, 25.0, -25.0, -25.0, List.empty))
    )
  }
  ...
  it should "update the color of a node" in withEditor { editor =>
    ...
    loaded.model.styleSheet
      .getNodeStyle(Some("1"))
      .flatMap(_.fill)
      .flatMap(_.color) should be(Some(color))
  }
}
